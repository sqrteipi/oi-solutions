...
struct SegTree {
  vi seg, lazy, arr;
  SegTree(int n, vi arr2) { seg.resize(4*n+100); lazy.resize(4*n+100); REP(i, n + 5) seg.pb(i), lazy.pb(0); arr = arr2;}
  void build(int ind, int l, int r) {
    if (l == r) seg[ind] = arr[l];
      ...
      seg[ind] = min(seg[ind * 2], seg[ind * 2 + 1]); // Function
    }
  }
  int query(int ind, int l, int r, int ql, int qr) {
    ...
    // Continue Visit
    int mid = (l + r) / 2;
    return min(query(ind * 2, l, mid, ql, qr), query(ind * 2 + 1, mid + 1, r, ql, qr)); // Function
  }
...
int32_t main(){
  ...
  vi a(n, 0);
  REP(i, n) cin >> a[i];
  auto seg = SegTree(n, a);
  seg.build(1, 0, n-1);
  // for (auto var:seg.seg) cout << var << " ";
  while (m--) {
    int l, r;
    cin >> l >> r;
